...
- Agent 团队：`chief_of_staff`、`product`、`cto`、`designer`、`backend`、`frontend`、`qa`、`devops`、`content`、`growth`、`sales`、`finance`、`compliance`、`coding`。所有 Agent 都按深耕行业多年的资深员工设定工作，强调行业洞察、严谨执行和跨职能协作；其中 `designer` 负责高级 UI 审美、Apple 风格参考、视觉层级、组件状态和设计 QA，帮助前端摆脱 AI 味。
